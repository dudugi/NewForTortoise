...
class CPictureEx : public CBCGPStatic
{
...
public: 
	...
	BOOL IsGIF() const;			//如果当前图像是 GIF 则返回 TRUE
	BOOL IsPlaying() const;		//如果当前图像是以动画方式显示则返回TRUE
	BOOL IsAnimatedGIF() const;	//如果当前图像是动画 GIF，则返回TRUE
	SIZE GetSize() const;		//返回图像尺寸
	int GetFrameCount() const;	//返回当前图像的帧数量 
	COLORREF GetBkColor() const; //获取当前填充颜色
	void SetBkColor(COLORREF clr); //为透明区域设置填充颜色
