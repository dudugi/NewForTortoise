...
typedef struct tag_GridCtrl_FieldData
{
	...
	shared_ptr<vector<CString>> sptr_normalComboxData;//普通下拉框控件用到的数据
	shared_ptr<vector<int>> sptr_cellBackgroundClrVec;//字段单元格背景颜色 vector为空使用内部默认颜色 yangjr 2017-6-13
...
//表格一行数据定义
typedef map<CString/*strColID 列名称ID*/,S_GridCtrl_FieldData/*列数据*/> MAP_GridCtrl_RowData;

//表格所有行数据定义
typedef map<int/*行号*/,MAP_GridCtrl_RowData/*表格一行数据*/> MAP_GridCtrl_AllData;
...
typedef map<CString/*strColID 列名称*/,S_GridCtrl_FieldAtrri> MAP_GridCtrl_FieldAtrri;

typedef map<int/*表格样式显示的行*/,CString/*strColID 列名称*/> MAP_GridCtrl_FieldPos;


#pragma region 表格属性

//gridctrl 表格属性
typedef struct tag_GridCtrl_TableAtrri 
{
...
typedef vector<SHAIXUAN_DATA> VEC_SHAIXUAN_DATA;//所有的筛选数据
...
typedef map<CString/*strColID 列名称*/,SERCH_DATA/*筛选栏数据*/> MAP_SERCH_DATAS;
/**********************表格内部使用数据 end*********************************/




#define GRID_COL_INDEX			_T("Grid_Index")//序号