...
int CLDK::IsOutTime( CString& strKdsworkPath, CString& strAppPath ,int& nOuttime)
{
	//xq define 20170823 ============================================== 
	CString strEndTime;
	CString strFilePath = strKdsworkPath +_T("\\运行库\\共用插件\\ordersvr.plw");//设置配置文件路径
 #ifdef _DEBUG
	if (!PathFileExists(strFilePath))
	{
		strFilePath = strAppPath +_T("\\ordersvr.plw");//设置配置文件路径
	}
...
int   CLDK::CheckData( CString& strKdsworkPath ,CString& strAppPath ,int& nOuttime , int& status )
{
	status = 0;

	GetHaspKey(HASP_PLMVERSION);

	IsOutTime( strKdsworkPath , strAppPath , nOuttime);
...
int   CLDK::CheckData1_3(CString& strKdsworkPath ,CString& strAppPath ,int& nOuttime, int& status  )
{
	CTime cTime = GetTime();
	if( cTime.GetMonth() >=1 && cTime.GetMonth() <= 3)
	{
		CheckData( strKdsworkPath ,strAppPath , nOuttime, status  );
	}
	return 0;
}

int   CLDK::CheckData9_12(CString& strKdsworkPath ,CString& strAppPath ,int& nOuttime, int& status  )
{
	CTime cTime = GetTime();
...
int   CLDK::CheckDataOnTime(  int& status )
{
	CTime cTime = GetTime();
	int nHour = rand()%3 + 1;		
...
void CLDK::WriteOutTimeFile( int nType, CString& strAppPath )
{
	GetHaspKey(HASP_SHELL);
	...
	CFile file;
	CString strFilePath = strAppPath +_T("\\UpDatasvr.kdw");//设置配置文件路径
	if( nType == 1)//PLM
	{		
		strFilePath = strAppPath +_T("\\ordersvr.plw");//设置配置文件路径
	}
