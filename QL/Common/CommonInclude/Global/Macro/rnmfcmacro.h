...
{//界面效果的命名空间
   ...
   //该值固定不能修改
#define DEFAULT_VOLID 0734
   //获取硬盘ID
   inline DWORD GetVolID()
   {
      DWORD dwVolumeSerialNumber;
      // 将硬盘序列号与运算子异或后做为用户号
      ::GetVolumeInformation(L"C:\\",NULL,NULL,&dwVolumeSerialNumber,NULL,NULL,NULL,NULL);  
      if (0 == dwVolumeSerialNumber)
      {
         dwVolumeSerialNumber = DEFAULT_VOLID;
      }
      //大于9位数则去掉高位数值 使之为9位数
      if (dwVolumeSerialNumber > 100000000)
      {
         dwVolumeSerialNumber -= dwVolumeSerialNumber/100000000 *100000000;
      }
