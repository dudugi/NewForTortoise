...
typedef enum {
   ...
   //单号的子单号
   FINIT(CHILD_CODE,STRINGPOINT,21),
   //拷贝时的子单号
   FINIT(DST_CHILD_CODE,STRINGPOINT,22),
...
typedef enum {
  ...
  FILETRANSINFO_PATH,//格式化后的路径
  FILETRANSINFO_DST_PATH,//格式化后的目标路径
...
typedef enum{
   ...
   en_Action_Move       ,//移动文件 或文件夹 实现与拷贝类似 参数为一个参数的vec
}FILETRANSACTION;
